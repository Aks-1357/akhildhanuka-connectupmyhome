...
<?php echo FSS_Helper::PageTitle("SUPPORT","INCLUDE_USER"); ?>
<div><?php echo JText::_('INCLUDE_USER_HELP'); ?></div>
<form action="<?php echo FSSRoute::x( 'index.php?option=com_fss&view=ticket&tmpl=component&what=pickccuser&ticketid=' . JRequest::getVar('ticketid') );?>" method="post" name="adminForm">
<div id="editcell">
	<table>
		<tr>
			<td width="100%">
				<?php echo JText::_("SEARCH"); ?>:
				<input type="text" name="search" id="filter" value="<?php echo JViewLegacy::escape($this->search);?>" onchange="document.adminForm.submit();"/><br>
				<button onclick="this.form.submit();"><?php echo JText::_("GO"); ?></button>
				<button onclick="document.getElementById('filter').value='';this.form.submit();this.form.getElementById('faq_cat_id').value='0';"><?php echo JText::_("RESET"); ?></button>
			</td>
		</tr>
	</table>
...
jQuery(document).ready(function () {
	jQuery('.pick_user').click(function(ev) { 
		ev.preventDefault();
		window.parent.AddCCUser(jQuery(this).attr('id').split('_')[1]);
	});
});
</script>