...
jQuery(document).ready(function () {
	jQuery('.fss_publish_button').click(function (ev) {
		ev.preventDefault();
<?php if ($this->content->permission['artperm'] > 2): ?>
		...
		if (current == 0)
		{
			...
			var url = '<?php echo FSSRoute::x('index.php?option=com_fss&view=admin&layout=content&type=' . $this->content->id . '&what=publish', false);?>&id=' + id;
...
<?php endif; ?>
			...
			var url = '<?php echo FSSRoute::x('index.php?option=com_fss&view=admin&layout=content&type=' . $this->content->id . '&what=unpublish&id=XXIDXX', false);?>';
