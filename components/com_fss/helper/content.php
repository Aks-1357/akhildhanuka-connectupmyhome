...
class FSS_ContentEdit
{
	...
	function Init()
	{
		if (empty($this->permission))
		{
			...
			$this->permission = FSS_Helper::getPermissions();		
	...
	function Display()
	{
		...
		if ($this->what == "cancel")
		{
			...
			$link = FSSRoute::x('index.php?option=com_fss&view=admin&layout=content&type=' . $this->id,false);
			$return = JRequest::getVar('return','');
			if ($return && $return != 1)
				$link = $return;
	...
	function Save()
	{
		...
		if ($ok)
		{
			...
			if ($this->what == "apply")
			{
				$link = FSSRoute::x("index.php?option=com_fss&view=admin&layout=content&type={$this->id}&what=edit&id={$this->articleid}",false);
			} elseif ($this->what == "savenew")
			{
				$link = FSSRoute::x("index.php?option=com_fss&view=admin&layout=content&type={$this->id}&what=new",false);
			} else {
				$link = FSSRoute::x('index.php?option=com_fss&view=admin&layout=content&type=' . $this->id,false);
				$return = JRequest::getVar('return','');
				if ($return && $return != 1)
	...
	function Header()
	{
		...
		echo FSS_Helper::PageTitle("SUPPORT_ADMIN","CONTENT_MANAGEMENT");

		include JPATH_SITE.DS.'components'.DS.'com_fss'.DS.'views'.DS.'admin'.DS.'snippet'.DS.'_tabbar.php';
		include JPATH_SITE.DS.'components'.DS.'com_fss'.DS.'views'.DS.'admin'.DS.'snippet'.DS.'_contentbar.php';
	...
	function GetLookupValues($field)
	{
		...
		if (property_exists($field, 'lookup_extra'))
		{
			...
			$this->lookup_values[$field->field] = array_merge($extra, $this->lookup_values[$field->field]);
		}
	...
	function HandleAuthor()
	{
		...
		// build query
		$qry = "SELECT * FROM #__fss_user as fsu LEFT JOIN #__users as u ON fsu.user_id = u.id";
		$where = array();
		$where[] = "fsu.artperm > 0";
		...
		if ($search != "")
		{
			$where[] = "(u.username LIKE '%".FSSJ3Helper::getEscaped($db, $search)."%' OR u.name LIKE '%".FSSJ3Helper::getEscaped($db, $search)."%' OR u.email LIKE '%".FSSJ3Helper::getEscaped($db, $search)."%')";
		}
	...
	function EditPanel($item,$changeclass = true)
	{
		...
		if ($this->permission['artperm'] > 1 || ($this->permission['artperm'] == 1 && $item['author'] == $this->userid)) {
			echo "<div class='fss_content_edit_article'>";
			echo "<a href='".FSSRoute::_("index.php?option=com_fss&view=admin&layout=content&type={$this->id}&what=edit&id={$item['id']}&option=com_fss&return=1")."' class='fsj_tip' title='".JText::_('EDIT_ARTICLE')."'><img src='" . JURI::root( true ). "/components/com_fss/assets/images/edit.png' alt='Edit' /></a>";	
			...
			if ($this->permission['artperm'] > 2) :
				?>
					<a href="#" id="publish_<?php echo $item['id']; ?>" class="fss_publish_button fsj_tip" state="<?php echo $item['published']; ?>" title="<?php echo $tip; ?>">
						<?php echo FSS_Helper::GetPublishedText($item['published'],true); ?>
					</a>
				<?php
			else:
				?>
					<?php echo str_replace("_16.png","_g_16.png",FSS_Helper::GetPublishedText($item['published'])); ?>
				<?php
			endif;
