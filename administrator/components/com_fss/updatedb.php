		...
		// COMPARE FIELDS
		{
			...
			foreach ($stuff['fields'] as $field)
			{
				...
				if (array_key_exists($fieldname,$existing))
				{
					...
					$db->SetQuery($change);
					$db->Query();
					//ALTER TABLE `jos_fss_ticket_field` ADD `gfda` INT( 4 ) NOT NULL 
				}
			}
		}
...
//##NOT_EXT_END##
	...
	function SortSettings()
	{
		...
		$db->Setquery("REPLACE INTO #__fss_settings_big (setting, value) SELECT * FROM #__fss_settings WHERE setting = 'display_popup'");	
		$db->Query();
		
		$db->SetQuery("DELETE FROM #__fss_settings WHERE setting = 'display_popup'");
		$db->Query();
	...
	function ValidateMenus()
	{
		...
		} elseif ($this->DBIs16())
		{
			...
			// find base item
			$db	= & JFactory::getDBO();
			$db->setQuery("SELECT * FROM #__components WHERE link = 'option=com_fss' AND parent = 0");
			$component = $db->loadObjectList();
			...
			if (file_exists(JPATH_COMPONENT.DS.'fss.xml'))
			{
				...
				foreach ($xml->administration->submenu->menu as $item)
				{
					...
					if (!$componentitem)
					{
						...
						$qry .= " '$name', $componentid, '$link', '$name', 'com_fss', $order, 'images/blank.png', 0, 1)";
						$db->setQuery($qry);$db->Query();
						$log .= "Adding menu item $name<Br>";
					} else {
						//print_r($componentitem);
	...
	function RelinkMenuItems()
	{
		...
		if ($this->DBIs16())
		{
			...
			$qry = "UPDATE #__menu SET componentid = $componentid WHERE link LIKE '%option=com_fss%'";
			$db->setQuery($qry);$db->Query();
			$count = $db->getAffectedRows();
			$log .= "Relinked $count menu items<br>";
		}
	...
	function SetupPluginDBEntries()
	{
		...
		if (!$this->DBIs16())
		{
			...
			$qry = "SELECT * FROM #__plugins WHERE folder = 'search' AND element = 'fss_announce'";
			$db->setQuery($qry);
			$item = $db->loadObject();
			if (!$item)
			{
				$qry = "INSERT INTO #__plugins (name, element, folder, iscore) VALUES ('Search - Freestyle Announcements','fss_announce','search',0)";
				$db->setQuery($qry);$db->Query();
				$log .= "Adding Announcements Search plugin<br>";
			}

			$qry = "SELECT * FROM #__plugins WHERE folder = 'search' AND element = 'fss_faqs'";
			$db->setQuery($qry);
			$item = $db->loadObject();
			if (!$item)
			{
				$qry = "INSERT INTO #__plugins (name, element, folder, iscore) VALUES ('Search - Freestyle FAQs','fss_faqs','search',0)";
				$db->setQuery($qry);$db->Query();
				$log .= "Adding FAQs Search plugin<br>";
			}

			$qry = "SELECT * FROM #__plugins WHERE folder = 'search' AND element = 'fss_kb'";
			$db->setQuery($qry);
			$item = $db->loadObject();
			if (!$item)
			{
				$qry = "INSERT INTO #__plugins (name, element, folder, iscore) VALUES ('Search - Freestyle Knowledge Base','fss_kb','search',0)";
				$db->setQuery($qry);$db->Query();
				$log .= "Adding Knowledge Base Search plugin<br>";
			}

			$qry = "SELECT * FROM #__plugins WHERE folder = 'system' AND element = 'fss_cron'";
			$db->setQuery($qry);
			$item = $db->loadObject();
			if (!$item)
			{
				$qry = "INSERT INTO #__plugins (name, element, folder, iscore) VALUES ('System - Freestyle CRON Plugin','fss_cron','system',0)";
				$db->setQuery($qry);$db->Query();
				$log .= "Adding Knowledge Base Search plugin<br>";
			}
			...
			if (!$item)
			{
				$qry = "INSERT INTO #__extensions (name, type, element, folder, enabled) VALUES ('System - Freestyle CRON Plugin','plugin','fss_cron','system', 0)";
				$db->setQuery($qry);$db->Query();
...
//##NOT_TEST_END##
	...
	function AddAdminUser()
	{
		...
		$qry = "SELECT * FROM #__fss_user WHERE user_id = '$id'";
		//echo $qry."<br>";
		$db->SetQuery($qry);
		...
		if (count ($row) == 0)
		{
			$qry = "INSERT INTO #__fss_user (mod_kb, mod_test, support, user_id, autoassignexc, allprods, allcats, alldepts, artperm, groups) VALUES ";
