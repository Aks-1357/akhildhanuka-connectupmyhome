 ...
 * Homeconnect model.
 */
class HomeconnectModelhomeconnect extends JModelAdmin
{
	/**
	 * @var		string	The prefix to use with controller messages.
	 * @since	1.6
	 */
	protected $text_prefix = 'COM_HOMECONNECT';
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_homeconnect.edit.homeconnect.data', array());
	...
	protected function prepareTable(&$table)
	{
		...
		if (empty($table->id)) {
			...
			if (@$table->ordering === '') {
				$db = JFactory::getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__homeconnect');
				$max = $db->loadResult();
